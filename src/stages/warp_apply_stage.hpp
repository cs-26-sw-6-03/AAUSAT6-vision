...
{
public:
    WarpApplyStage(std::shared_ptr<Router> router, const Config& cfg, int cpu_affinity = -1)
    ...
    void process(std::shared_ptr<FrameContext> ctx) override
    {
        if (ctx->ransac_result.has_value() && !ctx->ransac_result->homography.empty())
        {
            ...
            cv::Mat stabilized;
            cv::warpAffine(ctx->frame, stabilized, warp23,
                           ctx->frame.size(), cv::INTER_NEAREST, cv::BORDER_REPLICATE);
